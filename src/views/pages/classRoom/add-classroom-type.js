...
import { AllClassRoomsType, DeleteRoomTyp, EditClassRoomsType } from "../../../services/home/manageClassroom.js"
...
const ClassRoomType = () => {
  ...
  const [loader, setLoader] = useState(true)  
    ...
    const deleteClassRoomType = async (id) => {
      try {
        const response = await DeleteRoomTyp(id)
        if (response) {
          if (response.data) {
    ...
    function handleregis () {
      ...
      function checkValidArabicName(name) {
        ...
        if (!!nam && !!namArabic && status === 'add' && checkValid(nam) && checkValidArabicName(namArabic)) {
            const reqObj = {
                room_type_name: nam,
                room_type_name_arabic: namArabic,
                school_id: schoolId
            }
            authConfig.post(`/add_room_types`, reqObj, {
                        headers: Headers()
                    })
                    .then((response) => {
            ...
            authConfig.post(`/update_room_types`, reqObj, {
                        headers: Headers()
                    })
                    .then((response) => {
                        if (response && response.data) {
    ...
    const editClassRoomType = async (id) => {
        const response = await EditClassRoomsType(id)
        if (response) {
          if (response.data) {
            const { result } = response.data.data
            setNamArabic(result[0].room_type_name_arabic)
            setName(result[0].room_type_name)
            setStatus('update')
            setId(id)
            setNameValid(false)
            setNamArabicValid(false)
       ...
       <Form className={dynamicClass} >
        <Row className="justify-content-between">
            <Col xxl={6} xl={6} lg={6} md={12} sm={12} xs={12}>
                <FormGroup>
                    <Label for="name">Room Type Name (English)</Label>
                    <Input invalid={namValid} type="text" name="name" id="name" placeholder="Room Type Name in English" value={nam} onChange={handleName}/>
                    <FormFeedback>Please enter valid Name</FormFeedback>
                </FormGroup>
            </Col>
            <Col xxl={6} xl={6} lg={6} md={12} sm={12} xs={12}>
                <FormGroup>
                    <Label for="nameAr">Room Type Name (Arabic)</Label>
                    <Input invalid={namArabicValid} type="text" name="nameAr" id="nameAr" placeholder="Room Type Name in Arabic" value={namArabic} onChange={handleNameAr}/>
                    <FormFeedback>Please enter valid composed only with arabic letter</FormFeedback>
                </FormGroup>
            </Col> 
            <Col xxl={12} xl={12} lg={12} md={12} sm={12} xs={12}>
            <div className="d-flex justify-content-between mt-3">
            <div className=""><Button color='primary' onClick={handleregis}>{!!status && status === 'update' ? 'Update Room Type' : 'Add Room Type' }</Button></div>
