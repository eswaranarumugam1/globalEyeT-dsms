...
import { AllClassRoomsFunction, DeleteRoomFunction, EditClassRoomsFunction } from "../../../services/home/manageClassroom.js"
...
const ClassRoomFunction = () => {
  ...
  const [loader, setLoader] = useState(true)
    ...
    const deleteClassRoomFunction = async (id) => {
      try {
        const response = await DeleteRoomFunction(id)
        if (response) {
          if (response.data) {
    ...
    function handleregis () {
      ...
      function checkValidArabicName(name) {
        return /^[\u0621-\u064A0-9 ]+$/.test(name)
      }
        if (!!nam && !!namArabic && status === 'add' && checkValid(nam) && checkValidArabicName(namArabic)) {
            const reqObj = {
                function_name: nam,
                function_name_arabic: namArabic,
                school_id: schoolId
            }
            authConfig.post(`/add_room_functions`, reqObj, {
                        headers: Headers()
                    })
                    .then((response) => {
        ...
        } else if (!!nam && !!namArabic && status === 'update' && checkValid(nam) && checkValidArabicName(namArabic)) {
            const reqObj = {
                function_name: nam,
                function_name_arabic: namArabic,
                school_id: schoolId,
            ...
            authConfig.post(`/update_room_functions`, reqObj, {
                        headers: Headers()
                    })
                    .then((response) => {
                        if (response && response.data) {
    ...
    const editClassRoomFunction = async (id) => {
        const response = await EditClassRoomsFunction(id)
        if (response) {
          if (response.data) {
            const { result } = response.data.data
            setNamArabic(result[0].function_name_arabic)
            setName(result[0].function_name)
            setStatus('update')
            setId(id)
            setNameValid(false)
            setNamArabicValid(false)
       ...
       <Form className={dynamicClass} >
        <Row className="justify-content-between">
            <Col xxl={6} xl={6} lg={6} md={12} sm={12} xs={12}>
                <FormGroup>
                    <Label for="name">Room Function Name (English)</Label>
                    <Input invalid={namValid} type="text" name="name" id="name" placeholder="Room Function Name in English" value={nam} onChange={handleName}/>
                    <FormFeedback>Please enter valid Name</FormFeedback>
                </FormGroup>
            </Col>
            <Col xxl={6} xl={6} lg={6} md={12} sm={12} xs={12}>
                <FormGroup>
                    <Label for="nameAr">Room Function Name (Arabic)</Label>
                    <Input invalid={namArabicValid} type="text" name="nameAr" id="nameAr" placeholder="Room Function Name in Arabic" value={namArabic} onChange={handleNameAr}/>
                    <FormFeedback>Please enter valid Name</FormFeedback>
                </FormGroup>
            </Col> 
            <Col xxl={12} xl={12} lg={12} md={12} sm={12} xs={12}>
            <div className="d-flex justify-content-between mt-3">
            <div className=""><Button color='primary' onClick={handleregis}>{!!status && status === 'update' ? 'Update Room Function' : 'Add Room Function' }</Button></div>
            <div className=""><Button color='primary' onClick={handleClose}>Close</Button></div>
            </div>
            </Col>
            </Row>
            </Form>
...
<div></div><div></div><div></div>
            ...
            {!!items && items.length !== 0 ? <div className="row m-2">
            <Table responsive>
              <thead>
                <tr>
                    <th>Sl No</th>
            ...
            <div className="d-flex justify-content-center mt-5">
                  <div>No Data Found, Please Add First</div>
             </div>
                  </> }
      </div> 
      }
...
export default ClassRoomFunction