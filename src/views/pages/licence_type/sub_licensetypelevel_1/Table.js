...
import { AllSubLicenceType1, DeleteSubLicTyp1, next_SubLicenceType1 } from "../../../../services/home/LicensePage"
...
function App() {
  ...
  const getSublicense_type1 = async () => {
    const response = await AllSubLicenceType1(school_id)
    if (response) {
      if (response.data) {
  ...
  const fetchSublicense_type1 = async (currentPage) => {
    const response = await next_SubLicenceType1(currentPage, school_id)
    const { data: { result: { data } } } = response.data
    return data
  }
  ...
  const handlePageClick = async (data) => {
    ...
    const sublicense_typeFormServer = await fetchSublicense_type1(currentPage, school_id)
    setItems(sublicense_typeFormServer)
  }

  const deletesublicencetype1 = async (id) => {
    console.log(id)
    try {
      const response = await DeleteSubLicTyp1(id)
      if (response) {
        if (response.data) {
  ...
  return (
    <div className="container">
      ...
      <br/>
         <Link to={`/pages/sub_license_type_level1/create`}>
             <Button type="button" color="success">
                 Create New Sub Licence Type 1
             </Button>
         </Link>
         <br/><br/>
            <Table responsive>
                        ...
                        <DropdownMenu right>
                          <DropdownItem href='/' onClick={e => e.preventDefault()}>
                            <Edit className='mr-50' size={15} />
                              <Link to={`/pages/sub_license_type_level1/edit/${ele.id}`}>
                                <span className='align-middle'>Edit</span>
                              </Link>
                          </DropdownItem>
                          <DropdownItem href='/' onClick={e => e.preventDefault()}>
                            <Trash className='mr-50' size={15} /> <span className='align-middle' onClick={() => {
                                deletesublicencetype1(ele.id)
                              }}>Delete</span>
                          </DropdownItem>
                        </DropdownMenu>
                      </UncontrolledDropdown>
