...
import { UpdateSublicensetypes1, SubLicenseType, LicenseTypeget, getSubLicenceType1Id } from "../../../../services/home/LicensePage"
...
const edit = () => {
  ...
  const school_id = parseInt(localStorage.getItem("schoolIdToken"))
    ...
    const updateSubLicenseType1 = async () => {
      ...
      // console.log(name)
      // console.log(picture)
      // console.log(license_type_id)
      // console.log(parent_id)
        // const request = {
        //     name, image, license_type_id, parent_id
        //     }
            console.log(formData)
            console.log('id is', paramsFromRoute.id && paramsFromRoute.id)
            try {
                const response = await UpdateSublicensetypes1(paramsFromRoute.id && paramsFromRoute.id, formData)
                if (response) {
                if (response.data) {
    ...
    const getSubLicenceType1ByID = async (id) => {
    try {
        const response = await getSubLicenceType1Id(id)
        if (response.data) {
            const { data: { result } } = response.data
            console.log(result)
            if (result) {
                const { name, parent_id, license_type_id, image, description} = result
                setName(name)
    ...
    useEffect(() => {
    getLicenceType()
    getSubLicenceType1ByID(paramsFromRoute.id)
        }, [paramsFromRoute.id])
            ...
            <CardBody>
                <AvForm>
                    <Row>
                        ...
                        <Col sm='6'>
                            <AvGroup>
                                ...
                                <option value={''}>Select Sub License Type</option>
                                {
                                    ...
                                    <option value={parseInt(ele.id)}>{ele.name}</option>
                                    )) : null
                                }
                                </AvField>
                        ...
                        <Col sm='4'>
                            <Button color='primary' type='submit' onClick={updateSubLicenseType1} >
                                Submit
                            </Button>
                        </Col>
                        <Col sm='4'>
