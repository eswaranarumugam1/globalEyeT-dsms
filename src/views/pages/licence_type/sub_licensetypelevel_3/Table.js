...
import { AllSubLicenceType3, DeleteSubLicTyp3, next_SubLicenceType3 } from "../../../../services/home/LicensePage"
...
function App() {
  ...
  const getSublicense_type3 = async () => {
    const response = await AllSubLicenceType3(school_id)
    if (response) {
      if (response.data) {
  ...
  const fetchSublicense_type3 = async (currentPage) => {
    const response = await next_SubLicenceType3(currentPage, school_id)
    const { data: { result: { data } } } = response.data
    return data
  }
  ...
  const handlePageClick = async (data) => {
    ...
    const sublicense_typeFormServer = await fetchSublicense_type3(currentPage, school_id)
    setItems(sublicense_typeFormServer)
  }

  const deletesublicencetype3 = async (id) => {
    try {
      const response = await DeleteSubLicTyp3(id)
      if (response) {
        if (response.data) {
  ...
  return (
    <div className="container">
      ...
      <br/>
         <Link to={`/pages/sub_license_type_level3/create`}>
             <Button type="button" color="success">
                 Create New Sub Licence Type 3
             </Button>
         </Link>
         <br/><br/>
            <Table responsive>
                        ...
                        <DropdownMenu right>
                          <DropdownItem href='/' onClick={e => e.preventDefault()}>
                            <Edit className='mr-50' size={15} />
                              <Link to={`/pages/sub_license_type_level3/edit/${ele.id}`}>
                                <span className='align-middle'>Edit</span>
                              </Link>
                          </DropdownItem>
                          <DropdownItem href='/' onClick={e => e.preventDefault()}>
                            <Trash className='mr-50' size={15} /> <span className='align-middle' onClick={() => {
                                deletesublicencetype3(ele.id)
                              }}>Delete</span>
                          </DropdownItem>
                        </DropdownMenu>
                      </UncontrolledDropdown>
